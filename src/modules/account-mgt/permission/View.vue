    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span v-if="getPageGroud(scope.row.pageGroupSn)">{{ getPageGroud(scope.row.pageGroupSn).name }}</span>
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template slot-scope="scope">
          <el-checkbox
            ...
            v-if="getPageGroud(scope.row.pageGroupSn)[item.prop] === 1"
...
export default {
  ...
  methods: {
    ...
    getPageGroud (sn) {
      return this.pageGourps.find(val => val.sn === sn)
    }
  }
}
</script>
