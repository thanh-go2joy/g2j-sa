...
export default {
  ...
  methods: {
    ...
    async exportSaleIncentive () {
      ...
      try {
        const params = {
          incentiveDate: this.filter.incentiveDate,
          saleIncentiveHistorySn: Number(this.$route.query.status) === 1 ? this.$route.params.sn : null
        }
        const { data } = await exportsSaleIncentiveHistoryDetails(params)
