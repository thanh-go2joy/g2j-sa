...
<sa-section>
  <div class="apply-hotel-notice-segment" v-loading="ui.loadingNoticeToHotelList">
      ...
      >
        ...
        <el-table-column :label="$t('page.noticeApplyHotel.operations')" align="center" min-width="100">
          <template slot-scope="scope">
  ...
  methods: {
    ...
    async addDataApplyingHotelFaq () {
      ...
      if (this.filter.suggestionsHotel) {
        this.queryAddHotelApply.applyTarget = this.filter.applyTarget
        this.queryAddHotelApply.faqInformationSn = this.filter.faqInformationSn
        this.queryAddHotelApply.hotelSn = this.filter.suggestionsHotel
      } else {
        this.queryAddHotelApply.applyTarget = this.filter.applyTarget
        this.queryAddHotelApply.provinceSn = this.filter.provinceSn
        this.queryAddHotelApply.districtSn = this.filter.districtSn
        this.queryAddHotelApply.hotelSn = this.filter.suggestionsHotel
        this.queryAddHotelApply.faqInformationSn = this.filter.faqInformationSn
      }
      try {
