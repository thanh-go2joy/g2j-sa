    ...
    <div class="notice-to-hotel-create-segment" v-loading="ui.isLoadingNoticeToHotelCreate">
      ...
      >
        ...
        <el-form-item :label="$t('page.detail-reply.allow_to_reply')">
          <el-checkbox
            :true-label="trueCheck"
            :false-label="falseCheck"
            v-model="dataCreatelNoticeToHotel.answer"
          ></el-checkbox>
        </el-form-item>
        <el-form-item :label="$t('page.detail-reply.apply_for_hotel')" prop="hotelList">
          <template>
            <p>
              <span class="style-padding-text" v-if="applyTarget">{{
                getLabelApplyHotel(applyHotelString, applyTarget)
              }}</span>
              <span v-if="applyTarget === 4"> ({{ totalHotel }}) </span>
              <span v-on:click.capture="cacheDataForm()">
                <router-link
                  :to="{
                    name: 'create_apply_for_hotel',
                    query: { applyTarget: applyTarget !== 0 ? applyTarget : 1, layoutType: 1 }
                  }"
                >
        ...
        <el-form-item :label="$t('page.detail-reply.topic')" prop="title">
          <el-input
            ...
            :placeholder="$t('page.detail-reply.topic')"
          ></el-input>
        </el-form-item>
        <el-form-item :label="$t('page.detail-reply.brief_detail')">
          <el-input
            ...
            v-model="dataCreatelNoticeToHotel.shortContent"
          ></el-input>
        </el-form-item>
        ...
        <el-form-item style="text-align: right">
          ...
          <el-button
            type="success"
            size="large"
            @click="addNoticeToHotel('dataCreatelNoticeToHotel')"
          >
...
<script>
import { createNoticeToHotel, loadApplyingHotelFaq } from './api'
...
export default {
  ...
  data: (vm) => {
    return {
      ...
      cacheDataCreatelNoticeToHotel: [],
      totalHotel: '',
      file: '',
      imageUrl: '',
      rules: {
        type: [
          { required: true, message: vm.$t('page.noticeDetail.requiredType'), trigger: 'change' }
        ],
        hotelList: [
          { required: true, message: vm.$t('page.noticeDetail.requiredHotelList'), trigger: 'blur' }
        ],
        title: [
          { required: true, message: vm.$t('page.noticeDetail.requiredTitle'), trigger: 'blur' }
        ]
      },
      dataRes: {},
      trueCheck: 1,
  ...
  created () {
    ...
    this.applyTarget =
      this.$route.query && this.$route.query.applyTarget ? this.$route.query.applyTarget : 0
    // if (this.applyTarget !== 0) {
    this.onLoadListHotel()
    // }
  },
  mounted () {
    const getCacheDataCreatelNoticeToHotel =
      JSON.parse(localStorage.getItem('cacheDataCreatelNoticeToHotel')) || []
    if (getCacheDataCreatelNoticeToHotel.length > 0) {
      this.cacheDataCreatelNoticeToHotel = getCacheDataCreatelNoticeToHotel
      this.dataCreatelNoticeToHotel =
        getCacheDataCreatelNoticeToHotel[
          getCacheDataCreatelNoticeToHotel.length - 1
        ].dataCreatelNoticeToHotel
    }
  },
  methods: {
        ...
        try {
          ...
          try {
            ...
            if (data?.code === repStatus.sussess) {
              ...
              localStorage.removeItem('cacheDataCreatelNoticeToHotel')
            }
          } catch (error) {
            return false
          } finally {
    ...
    cacheDataForm () {
      this.cacheDataCreatelNoticeToHotel.push({
        dataCreatelNoticeToHotel: this.dataCreatelNoticeToHotel
      })
      localStorage.setItem(
        'cacheDataCreatelNoticeToHotel',
        JSON.stringify(this.cacheDataCreatelNoticeToHotel)
      )
      this.dataCreatelNoticeToHotel = {}
    },
    async onLoadListHotel () {
      this.ui.isLoadingNoticeToHotelCreate = true
      try {
        const { data } = await loadApplyingHotelFaq()
