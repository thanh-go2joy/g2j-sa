<template>
  <sa-section class="notice-to-hotel-segment" v-loading="ui.loadingNoticeToHotelList">
    <template slot="header" class="notice-to-hdivotel-segment__filter">
      <el-form :inline="true" :model="query" class="demo-form-inline float-left">
        <el-form-item>
          <el-input
      ...
      <div class="notice-to-hotel-segment__filter--btn">
        <el-button type="warning" icon="el-icon-plus" @click="onDirectAddNoticeToHotel" plain>
          {{ $t('button.add') }}
        </el-button>
      </div>
    </template>
    ...
    >
      ...
      <el-table-column :label="$t('page.noticeList.apply_for_hotel')" min-width="100">
        <template slot-scope="scope">
          <span
            v-html="
              getLabelApplyNotice(
                scope.row.hotelScope,
                scope.row.numNotAnswer,
                scope.row.sn,
                scope.row.faqHotelCount
              )
            "
          ></span>
        </template>
      ...
      <el-table-column :label="$t('page.noticeList.operations')" align="center" min-width="100">
        <template slot-scope="scope">
...
import { getNoticeToHotelList, deleteNoticeToHotelList, resetApplyingHotelFaq } from './api'
...
export default {
  ...
  methods: {
    async getDataNoticeToHotelList (params) {
      ...
      try {
        const { data } = await getNoticeToHotelList(filter)
    ...
    onDirectAddNoticeToHotel () {
      if (localStorage.getItem('cacheDataCreatelNoticeToHotel')) {
        localStorage.getItem('cacheDataCreatelNoticeToHotel')
      }
      this.$router.push({
    ...
    getLabelApplyNotice (hotelScope, numNotAnswer, sn, faqHotelCount) {
      ...
      if (hotelScope < 4 && numNotAnswer <= 0) {
        ...
        switch (hotelScope) {
          case 1:
            return (
              this.$t('option.applyHotelOption.all_hotel') +
          ...
          case 2:
            return (
              this.$t('option.applyHotelOption.all_contracted') +
              '<span style="color:red"> (' +
              numNotAnswer +
              ' )</span>'
            )
          case 3:
            return (
              this.$t('option.applyHotelOption.all_trial') +
              '<span style="color:red"> (' +
              numNotAnswer +
              ' )</span>'
            )
        ...
        return (
          `<a href="list-apply-for-hotel?id=${sn}&targetSn=4" style="color: #409EFF">` +
          faqHotelCount +
          '</span></a>' +
          '<span style="color:red"> (' +
          numNotAnswer +
    ...
    async onResetListHotel () {
      this.ui.loadingNoticeToHotelList = true
      try {
        const { data } = await resetApplyingHotelFaq()
        this.dataRes = data
      } catch (error) {
        return false
      } finally {
