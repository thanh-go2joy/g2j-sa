  ...
  >
    ...
    >
      <div class="article-form__image">
        ...
        <br>
        <p v-if="handleImage.srcImage">Dung lượng ảnh {{ size }} Kb</p>
        <p style="color:red"> (kích thước hình ảnh không vượt quá 524 kb)</p>
        <image-uploader
          :preview="false"
...
export default {
  ...
  methods: {
    ...
    resultThumbnail (result) {
      ...
      this.size = (result.size * 0.0009765625).toFixed()
      this.handleImage.srcImage = URL.createObjectURL(result)
      this.formData.imagePath = result = URL.createObjectURL(result)
    },
    ...
    submitForm () {
      this.$refs.articleForm.validate(async (valid) => {
        ...
        try {
          ...
          let message = ''
          if (this.action === 'edit') {
            const { data } = await updateArticle(this.formData.sn, formData)
