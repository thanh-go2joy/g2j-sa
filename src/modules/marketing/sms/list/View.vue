<template>
  <sa-section class="sms">
    ...
    <el-table v-loading="ui.loading" :data="listSms">
      <el-table-column label="#" type="index" min-width="20"> </el-table-column>
      <el-table-column :label="$t('page.sms.sms_type')" min-width="120">
        <template slot-scope="scope">
          {{ scope.row.type === 1 ? $t('page.sms.custom_care') : $t('page.sms.marketing') }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('page.sms.sender')" min-width="100">
        <template slot-scope="scope">
      ...
      <el-table-column prop="content" :label="$t('page.sms.content')" min-width="250" />
      <el-table-column prop="createStaff" :label="$t('page.sms.create_staff')" min-width="100">
        <template slot-scope="scope">
          {{ scope.row.createStaff.fullName }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('page.sms.create_time')" min-width="160">
        <template slot-scope="scope">
          {{ formatDate(scope.row.createTime) }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('page.sms.send_to')" min-width="100">
        <template slot-scope="scope">
          ...
          <span v-else>
            ...
            <span v-if="scope.row.sendToContent.originalFileName">
              <a :href="exportExel(scope.row.sendToContent.filePath)" target="_blank">
      ...
      <el-table-column prop="numOfSMS" :label="$t('page.sms.num_of_sms')" min-width="80" />
      <el-table-column prop="totalPrice" :label="$t('page.sms.total_price')" min-width="80" />
      <el-table-column :label="$t('page.sms.send_schedule')" min-width="80">
        <template slot-scope="scope">
          {{ formatDate(scope.row.scheduleTime) }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('page.sms.send_time')" min-width="80">
        <template slot-scope="scope">
