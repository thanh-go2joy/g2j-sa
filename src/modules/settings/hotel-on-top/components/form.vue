  ...
  >
    ...
    <div class="hotel-on-top__form">
      ...
      <el-table-draggable handle="td">
        ...
        >
          <el-table-column
            :label="$t('page.settings.topHotel')"
            min-width="30"
          >
          ...
          <el-table-column
            :label="$t('page.settings.dateTime')"
            min-width="90"
          >
            <template slot-scope="scope">
              <el-date-picker
                :picker-options="pickerOptionsDate"
                v-model="formData.hotelOnTopDetails[scope.$index].dateTimePicker"
                type="datetimerange"
                range-separator="-"
                format="yyyy-MM-dd HH:mm"
                value-format="yyyy-MM-dd HH:mm"
        ...
        <el-table-column
          ...
          >
            <template slot-scope="scope">
              <el-button
                  class="w-100"
                  @click="removeTopHotel(scope.$index)"
                  type="danger"
                >
                  {{ $t('button.delete') }}
                </el-button>
...
export default {
  ...
  methods: {
    onChangeClassName (data) {
      return ''
      // return data.row.sn === 3169 ? 'disable-action' : ''
    },
    onChangeType (type) {
      if (type === 4) {
    ...
    submitForm () {
      this.$refs.form.validate(async (valid) => {
        ...
        try {
          ...
          this.formData.hotelOnTopDetails.map((item, index) => {
            item.idx = index
