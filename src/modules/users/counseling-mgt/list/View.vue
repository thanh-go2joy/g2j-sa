  ...
  methods: {
    async listCounseling () {
      ...
      try {
        ...
        params = {
          ...this.filter,
          ...this.listProvincesSn.length > 0 ? { provinceSn: this.listProvincesSn.toString() } : {}
        }
        const { data } = await listCounseling(params)
    ...
    handleCheckAllProvince (val) {
      ...
      this.listProvincesSn = val ? this.ui.provinceOptions : []
      this.listCounseling()
    },
    onSizeChange (size) {
